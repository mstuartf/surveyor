...
module.exports = {
  Query: {
    ...
    questions: async (_, {surveyId, userId}, { dataSources }) => dataSources.questions.getForSurvey(surveyId, userId),
    answers: async (_, {sessionId, questionId}, { dataSources }) => dataSources.answers.get(sessionId, questionId),
    info: async (_, {sessionId}, { dataSources }) => {
      ...
      const questions = await dataSources.questions.getForSurvey(survey.dataValues.id);
      ...
      return {
        ...
        questions: questions.map(question => ({
          ...question.dataValues,
          answers: answers
            .filter(answer => answer.dataValues.questionId === question.dataValues.id)
            .map(answer => ({
              ...answer.dataValues
            }))
        }))
  ...
  Mutation: {
    createAnswer: async (_, {sessionId, questionId, value}, { dataSources }) => {
      const answer = await dataSources.answers.create(sessionId, questionId, value);
      return {
        success: true,
